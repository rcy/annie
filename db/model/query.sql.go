...
const addOpenGraphData = `-- name: AddOpenGraphData :one
insert into opengraphs(url, data) values(?,?) returning created_at, url, data
`

type AddOpenGraphDataParams struct {
	Url  string
	Data interface{}
}

func (q *Queries) AddOpenGraphData(ctx context.Context, arg AddOpenGraphDataParams) (Opengraph, error) {
	row := q.db.QueryRowContext(ctx, addOpenGraphData, arg.Url, arg.Data)
	var i Opengraph
	err := row.Scan(&i.CreatedAt, &i.Url, &i.Data)
	return i, err
}
