...
func NewsByTopic(ctx context.Context, topic string, length int, model string) (string, error) {
	headlines, err := cnnHeadlines()
	if err != nil {
		return "", err
	}

	completion, err := ai.Complete(ctx, model,
		fmt.Sprintf("Summarize the following headlines in %d characters or less.", length),
		fmt.Sprintf("Only consider headlines pertaining to the topic: %s\n\nHeadlines follow: %s\n",
			topic,
...
func News(ctx context.Context, length int, model string) (string, error) {
	headlines, err := cnnHeadlines()
	if err != nil {
		return "", err
	}
